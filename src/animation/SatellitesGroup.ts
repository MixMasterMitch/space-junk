...
export default class SatellitesGroup extends SceneComponent {
    private static NUM_SATELLITES = 25000;
    private static NUM_TAIL_SEGMENTS = 20;
    private static NUM_TAIL_TRIANGLES = SatellitesGroup.NUM_TAIL_SEGMENTS + 1;
    private static NUM_TAIL_VERTICES = SatellitesGroup.NUM_TAIL_SEGMENTS + 3;

    private satelliteData?: Float32Array;
    ...
    public async initialize(scene: Scene, renderer: Renderer): Promise<void> {
        ...
        // this.satelliteData = [];
        // for (let i = 0; i < SatellitesGroup.NUM_SATELLITES; i++) {
        ...
        sphereGeometry.instanceCount = SatellitesGroup.NUM_SATELLITES;
        sphereGeometry.setAttribute('translation', new InstancedBufferAttribute(new Float32Array(SatellitesGroup.NUM_SATELLITES * 3), 3));
        ...
        // trailGeometry.setAttribute('position', new BufferAttribute(new Float32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_VERTICES * 3), 3));
        // trailGeometry.setAttribute('previous', new BufferAttribute(new Float32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_VERTICES * 3), 3));
        // trailGeometry.setAttribute('sunPosition', new BufferAttribute(new Float32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_VERTICES * 3), 3));
        // const sideArray = new Float32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_VERTICES);
        // trailGeometry.setAttribute('side', new BufferAttribute(sideArray, 1));
        // for (let i = 0; i < SatellitesGroup.NUM_SATELLITES; i++) {
        //     const offset = i * SatellitesGroup.NUM_TAIL_VERTICES;
        //     for (let j = 0; j < SatellitesGroup.NUM_TAIL_VERTICES; j++) {
        //         sideArray[offset + j * 2] = i % 2 === 0 ? 1 : -1;
        //     }
        // }
        // const widthArray = new Float32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_VERTICES);
        // trailGeometry.setAttribute('width', new BufferAttribute(widthArray, 1));
        // for (let i = 0; i < SatellitesGroup.NUM_SATELLITES; i++) {
        //     const offset = i * SatellitesGroup.NUM_TAIL_VERTICES;
        //     widthArray[offset] = 0;
        //     for (let j = 0; j < SatellitesGroup.NUM_TAIL_VERTICES - 2; j++) {
        //         widthArray[offset + 1 + j] = j / (SatellitesGroup.NUM_TAIL_VERTICES - 3);
        //     }
        //     widthArray[offset + SatellitesGroup.NUM_TAIL_VERTICES - 1] = 1;
        // }
        // const indexArray = new Uint32Array(SatellitesGroup.NUM_SATELLITES * SatellitesGroup.NUM_TAIL_TRIANGLES * 3);
        // trailGeometry.setIndex(new BufferAttribute(indexArray, 1));
        // for (let i = 0; i < SatellitesGroup.NUM_SATELLITES; i++) {
        //     const offset = i * SatellitesGroup.NUM_TAIL_TRIANGLES * 3;
        //     const vertexOffset = i * SatellitesGroup.NUM_TAIL_VERTICES;
        //     for (let j = 0; j < SatellitesGroup.NUM_TAIL_TRIANGLES; j++) {
    ...
    public updateSatelliteData(satelliteData: Float32Array): Float32Array | undefined {
        const prevSatelliteData = this.satelliteData;
        this.satelliteData = satelliteData;
        if (this.spheres !== undefined) {
            (this.spheres.geometry.attributes.translation.array as Float32Array) = this.satelliteData;
            this.spheres.geometry.attributes.translation.needsUpdate = true;
        }
        return prevSatelliteData;
    }
    ...
    public render(date: Date, camera: Camera, guiData: GUIData): void {
        ...
        // const advanceTrail =
        //     this.trailTimestamps.length < SatellitesGroup.NUM_TAIL_TRIANGLES ||
        //     date.getTime() - this.trailTimestamps[this.trailTimestamps.length - 1] >= (guiData.tailLength * 60 * 1000) / (SatellitesGroup.NUM_TAIL_TRIANGLES - 1);
        // if (advanceTrail) {
        //     this.trailTimestamps.push(date.getTime());
        //     if (this.trailTimestamps.length > SatellitesGroup.NUM_TAIL_TRIANGLES) {
        ...
        //
        //     const l = positionArray.length / SatellitesGroup.NUM_SATELLITES;
        //     const offset = i * SatellitesGroup.NUM_TAIL_VERTICES * 3;
        //
